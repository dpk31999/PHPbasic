		...
		if(size_img_avt > 5242880){
			...
			$(this).ajaxSubmit({
				...
				error: function(){
					$('#formUpAvt button[type="submit"]').html('Upload');
					$('#formUpAvt .alert-danger').removeClass('hidden');
					$('#formUpAvt .alert-danger').html('Có lỗi vui lòng thử lại sau');
				},
				resetForm: true
			});
			return false;
...
$('#del_avt').on('click',function(){
	$confirm = confirm('Bạn có muốn xóa ảnh đại diện ?');
	if($confirm == true){
		$.ajax({
...
$('#change_pass button').on('click',function(){
	$('#change_pass button').html('đang thay đổi...')
	...
	$re_new_pass = $('#re_new_pass').val();
		$.ajax({
			...
			success: function(data){
				$('#change_pass .alert').removeClass('hidden');
				$('#change_pass .alert').html(data);
				$('#change_pass button').html('Đổi mật khẩu');
			},
			error: function(){
				$('#change_pass .alert').removeClass('hidden');
				$('#change_pass .alert').html('Có lỗi vui lòng thử lại sau');
			}
		});
});
